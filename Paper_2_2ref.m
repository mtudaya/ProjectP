...
theta = [-30 0  ]; % angle of arival of the reflectors theta = randi([-90 90],1,L);
...
x_BPD_CVX=zeros(ND,M);
    for m = 1:1:M
        ...
        cvx_begin
            ...
            %minimize (lambda* norm(x0,1)+ norm(y-As*x0))
...
s=scatter(plot_x,error_alpha(1,:),90);hold on;
s.LineWidth = 0.8;
s.MarkerEdgeColor = 'k';
s.MarkerFaceColor = [1 0.0 0.0];

s=scatter(plot_x,error_alpha_L2(1,:),90);
s.LineWidth = 0.8;
s.MarkerEdgeColor = 'k';
...
%matlab2tikz('figurehandle',f1,'filename','alpha1.tex' ,'standalone', true);

f2 = figure;

plot_x=1:1:max_iteration;
s=scatter(plot_x,error_alpha(2,:),90);hold on;
s.LineWidth = 0.8;
s.MarkerEdgeColor = 'k';
s.MarkerFaceColor = [1 0.0 0.0];

s=scatter(plot_x,error_alpha_L2(2,:),90);
s.LineWidth = 0.8;
s.MarkerEdgeColor = 'k';
s.MarkerFaceColor = [0 0 1];
%plot (plot_x,error_alpha(1,:),'color','r','LineWidth',2);hold on;
...
ylim([0 60])
set(gca,'fontsize',28)
%matlab2tikz('figurehandle',f2,'filename','alpha2.tex' ,'standalone', true);
...
xlabel('% Error in amplitude estimation of 1st reflector')
legend('L1','L2')
set(gca,'fontsize',28, 'XScale', 'log')

xticks([ 0.1 1 10 100 1000])
xticklabels({'0.1','1','10','100','1000'})
set( h(1,1), 'LineStyle', '-', 'Color', 'r','LineWidth',1.0);
set( h(2,1), 'LineStyle', '-', 'Color', 'k','LineWidth',1.0);
%matlab2tikz('figurehandle',f6,'filename','alpha111CDF.tex' ,'standalone', true);
...
set( h(2,2), 'LineStyle', '-', 'Color', 'k','LineWidth',1.5);
matlab2tikz('figurehandle',f7,'filename','alpha21CDF.tex' ,'standalone', true);

      
f8 = figure;
h(1, 2) = cdfplot(error_tau(1,:)) ;hold on; %%L1
h(2, 2) = cdfplot(error_tau_L2(2,:)); % L2
xlabel('% Error in delay estimation of second reflector')
legend('L1','L2')
f12 = figure;
h(1, 2) = cdfplot(error_tau(2,:)) ;hold on; %%L1
h(2, 2) = cdfplot(error_tau_L2(2,:)); % L2
xlabel('% Error in delay estimation of second reflector')
legend('L1','L2')
