...
channel_energy_total=diag(h*h');
shi_length=length(channel_energy_total);
shi=zeros(shi_length-1,shi_length);
for n=1:1:shi_length-1
    shi(n,n)=0.25;
    shi(n,n+1)=-0.25;
end
shi_new=shi*channel_energy_total;
h_len = size(h,1);
...
fclose(fid);
plot(t,channel_energy_total);hold on;
plot(shi_new);
