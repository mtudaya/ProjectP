...
theta = [-30 -40 -60 -10 -20 -30];% angle of arival of the reflectors %theta = randi([-90 90],1,L);
theta_all(:,iteration) = transpose(theta);
theta_rad = deg2rad(theta);
v = [180 180 180 180 180 180]; % phase rotation due to reflection %v = randi([0 360],1,L);
...
h(2, 2) = cdfplot(error_tau_L2(1,:)); % L2
...
legend('L1','L2')
f17 = figure;
h(1, 2) = cdfplot(error_tau(3,:)) ;hold on; %%L1
h(2, 2) = cdfplot(error_tau_L2(3,:)); % L2
xlabel('Error in delay estimation of 3rd reflector (ns)')
legend('L1','L2')
f18 = figure;
h(1, 2) = cdfplot(error_tau(4,:)) ;hold on; %%L1
h(2, 2) = cdfplot(error_tau_L2(4,:)); % L2
xlabel('Error in delay estimation of 4th reflector (ns)')
legend('L1','L2')
f19 = figure;
h(1, 2) = cdfplot(error_tau(5,:)) ;hold on; %%L1
h(2, 2) = cdfplot(error_tau_L2(5,:)); % L2
xlabel('Error in delay estimation of 5th reflector (ns)')
legend('L1','L2')
f20 = figure;
h(1, 2) = cdfplot(error_tau(6,:)) ;hold on; %%L1
h(2, 2) = cdfplot(error_tau_L2(6,:)); % L2
