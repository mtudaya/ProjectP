...
alpha = sort (0.2 + (1-0.2).*rand(L,1),'descend'); % reflection coefficients of the reflectors  r = a + (b-a).*rand(N,1)
alpha_all(:,iteration) =(alpha); 
theta = [-30 -20 -50 -20 ];% angle of arival of the reflectors %theta = randi([-90 90],1,L);
theta_all(:,iteration) = transpose(theta);
theta_rad = deg2rad(theta);
v = [180 180 180 180 ]; % phase rotation due to reflection %v = randi([0 360],1,L);
v_all(:,iteration) = transpose(v);
v_rad=deg2rad(v);
Tau = [(0.25*Ts)  (0.35*Ts)  (1.85*Ts) (1.05*Ts)];
...
plot (plot_x,error_alpha(1,:),'color','r');hold on;
plot (plot_x,error_alpha_L2(1,:));

f2 = figure;

plot_x=1:1:max_iteration;
plot (plot_x,error_alpha(2,:),'color','r');hold on;
plot (plot_x,error_alpha_L2(2,:));

f3 = figure;

plot_x=1:1:max_iteration;
plot (plot_x,error_alpha(3,:),'color','r');hold on;
plot (plot_x,error_alpha_L2(3,:));

f4 = figure;

plot_x=1:1:max_iteration;
plot (plot_x,error_alpha(4,:),'color','r');hold on;
plot (plot_x,error_alpha_L2(4,:));
...
xlabel('% Error in amplitude estimation of third reflector')
ylabel('CDF')
legend('L1','L2')
f12 = figure;
h(1, 4) = cdfplot(error_alpha(4,:)) ;hold on; %L1
h(2, 4) = cdfplot(error_alpha_L2(4,:)); % L2
xlabel('% Error in amplitude estimation of fouth reflector')
ylabel('CDF')
legend('L1','L2')
