...
v_rad=deg2rad(v);
Tau =  (sort (0 + (2*Ts-0).*rand(L,1),'ascend')); %[(0.40*Ts)    (1.90*Ts) ];
...
error_tau(:,iteration) =abs((Tau)-tau_selected)*10^9;
error_theta(:,iteration) =abs((theta)-theta_seleted);
disp(iteration);
...
error_tau_L2(:,iteration) =abs((Tau)-tau_selected_L2)*10^9;
error_theta_L2(:,iteration) =abs((theta)-theta_seleted_L2);
disp(iteration);
