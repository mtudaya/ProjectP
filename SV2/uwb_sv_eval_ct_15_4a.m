...
% any patents if so.
% Our intension is to help wireless community to do testing of their
% development with these channel models only.
function [h,h_ct,t,t_ct,ts,actual_cluster_times,Pcluster_actual,actual_ray_times,num_channels]=uwb_sv_eval_ct_15_4a(num_clusters_fc);
...
cm_num = 26;  % channel model number from 1 to 8
...
[h_ct,t_ct,t0,np,actual_cluster_times,Pcluster_actual,actual_ray_times] = uwb_sv_model_ct_15_4a(Lam,lambda,Lmean,lambda_mode,lambda_1, ...
...
figure(8); clf; plot(t, 10*log10(abs(h).^2)); grid on
title('POWER')
xlabel('Time (nS)')
%}
...
%plot(t_ct,h_ct); hold on;
%plot(t, abs(h)); 

%figure(8); clf; plot(t, 10*log10(abs(h).^2)); grid on; hold on;
%plot(t_ct,10*log10(abs(h_ct).^2));
%title('POWER')
%xlabel('Time (nS)')
%yy1 = smooth(t,10*log10(abs(h).^2),0.05,'rloess');
%plot(t, 10*log10(abs(h).^2)); grid on; hold on;
%plot(t,yy1)
