...
L =10; % number of reflectrots
N =100; %length of the channel (-N to N)
d = lambda_em/2; % antenna spacing of the antenna array
Tau_min=0;
Tau_max=4*Ts;
ND = 16; %resolution of the dictionary
...
theta = [-30 -40 -60 -10 -20 -30 -15 -25 -45 -10 ];% angle of arival of the reflectors %theta = randi([-90 90],1,L);
theta_all(:,iteration) = transpose(theta);
theta_rad = deg2rad(theta);
v = [180 180 180 180 180 180 180 180 180 180]; % phase rotation due to reflection %v = randi([0 360],1,L);
v_all(:,iteration) = transpose(v);
v_rad=deg2rad(v);
Tau = [(0.35*Ts) (0.55*Ts) (0.75*Ts) (1.05*Ts) (1.35*Ts) (1.55*Ts) (2.05*Ts) (2.75*Ts) (3.35*Ts) (3.85*Ts)];
Tau_all(:,iteration) = transpose(Tau);




 gamma = ones(M,L);% phase dely due to antenna array
...
error_tau(:,iteration) =abs(transpose(Tau)-tau_selected)*10^9;
error_theta(:,iteration) =abs(transpose(theta)-theta_seleted);
disp(iteration);
...
error_tau_L2(:,iteration) =abs(transpose(Tau)-tau_selected_L2)*10^9;
error_theta_L2(:,iteration) =abs(transpose(theta)-theta_seleted_L2);
disp(iteration);
...
f7 = figure;
plot_x=1:1:max_iteration;
plot (plot_x,error_alpha(7,:),'color','r','LineWidth',2);hold on;
plot (plot_x,error_alpha_L2(7,:),'color','k','LineWidth',2);
xlabel('Number of iterations')
ylabel('% Error in amplitude estimation of 7th reflector')
legend('L1','L2')
set(gca,'fontsize',16)
...
f8 = figure;
plot_x=1:1:max_iteration;
plot (plot_x,error_alpha(8,:),'color','r','LineWidth',2);hold on;
plot (plot_x,error_alpha_L2(8,:),'color','k','LineWidth',2);
xlabel('Number of iterations')
ylabel('% Error in amplitude estimation of 8th reflector')
legend('L1','L2')
set(gca,'fontsize',16)
...
f9 = figure;
plot_x=1:1:max_iteration;
plot (plot_x,error_alpha(9,:),'color','r','LineWidth',2);hold on;
plot (plot_x,error_alpha_L2(9,:),'color','k','LineWidth',2);
xlabel('Number of iterations')
ylabel('% Error in amplitude estimation of 9th reflector')
legend('L1','L2')
set(gca,'fontsize',16)
...
f10 = figure;
plot_x=1:1:max_iteration;
plot (plot_x,error_alpha(10,:),'color','r','LineWidth',2);hold on;
plot (plot_x,error_alpha_L2(10,:),'color','k','LineWidth',2);
xlabel('Number of iterations')
ylabel('% Error in amplitude estimation of 10th reflector')
legend('L1','L2')
set(gca,'fontsize',16)
...
f17 = figure;
h(1, 7) = cdfplot(error_alpha(7,:)) ;hold on; %L1
h(2, 7) = cdfplot(error_alpha_L2(7,:)); % L2
xlabel('% Error in amplitude estimation of 7th reflector')
     
f18 = figure;
h(1, 8) = cdfplot(error_alpha(8,:)) ;hold on; %L1
h(2, 8) = cdfplot(error_alpha_L2(8,:)); % L2
xlabel('% Error in amplitude estimation of 8th reflector')
     
f19 = figure;
h(1, 9) = cdfplot(error_alpha(9,:)) ;hold on; %L1
h(2, 9) = cdfplot(error_alpha_L2(9,:)); % L2
xlabel('% Error in amplitude estimation of 9th reflector')
     
f20 = figure;
h(1, 10) = cdfplot(error_alpha(10,:)) ;hold on; %L1
h(2, 10) = cdfplot(error_alpha_L2(10,:)); % L2
xlabel('% Error in amplitude estimation of 10th reflector')
legend('L1','L2')
set(gca,'fontsize',16)
    
f21 = figure;
h(1, 11) = cdfplot(error_tau(1,:)) ;hold on; %%L1
h(2, 11) = cdfplot(error_tau_L2(1,:)); % L2
xlabel('Error in delay estimation of first reflector (ns)')
legend('L1','L2')
f22 = figure;
h(1, 12) = cdfplot(error_tau(2,:)) ;hold on; %%L1
h(2, 12) = cdfplot(error_tau_L2(2,:)); % L2
xlabel('Error in delay estimation of second reflector (ns)')
legend('L1','L2')
f23 = figure;
h(1, 13) = cdfplot(error_tau(3,:)) ;hold on; %%L1
h(2, 13) = cdfplot(error_tau_L2(3,:)); % L2
xlabel('Error in delay estimation of 3rd reflector (ns)')
legend('L1','L2')
f24 = figure;
h(1, 14) = cdfplot(error_tau(4,:)) ;hold on; %%L1
h(2, 14) = cdfplot(error_tau_L2(4,:)); % L2
xlabel('Error in delay estimation of 4th reflector (ns)')
legend('L1','L2')
f25 = figure;
h(1, 15) = cdfplot(error_tau(5,:)) ;hold on; %%L1
h(2, 15) = cdfplot(error_tau_L2(5,:)); % L2
xlabel('Error in delay estimation of 5th reflector (ns)')
legend('L1','L2')
f26 = figure;
h(1, 16) = cdfplot(error_tau(6,:)) ;hold on; %%L1
h(2, 16) = cdfplot(error_tau_L2(6,:)); % L2
xlabel('Error in delay estimation of 6th reflector (ns)')
legend('L1','L2')

f27 = figure;
h(1, 17) = cdfplot(error_tau(7,:)) ;hold on; %%L1
h(2, 17) = cdfplot(error_tau_L2(7,:)); % L2
xlabel('Error in delay estimation of 7th reflector (ns)')
legend('L1','L2')

f28 = figure;
h(1, 18) = cdfplot(error_tau(8,:)) ;hold on; %%L1
h(2, 18) = cdfplot(error_tau_L2(8,:)); % L2
xlabel('Error in delay estimation of 8th reflector (ns)')
legend('L1','L2')

f29 = figure;
h(1, 19) = cdfplot(error_tau(9,:)) ;hold on; %%L1
h(2, 19) = cdfplot(error_tau_L2(9,:)); % L2
xlabel('Error in delay estimation of 9th reflector (ns)')
legend('L1','L2')

f30 = figure;
h(1, 20) = cdfplot(error_tau(10,:)) ;hold on; %%L1
h(2, 20) = cdfplot(error_tau_L2(10,:)); % L2
xlabel('Error in delay estimation of 10th reflector (ns)')
legend('L1','L2')
